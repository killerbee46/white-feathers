const productPrice = ({ productData, goldPrice,gold_tax=0, silverPrice, diamondPrice, details = false,carts,wishes }) => {

    const metal = productData?.Metal

    const metalPrice = (productData?.pmt_id <= 0 ? 0 : productData?.pmt_id < 11 ? goldPrice * metal?.purity / 100 / 11.664 : (productData?.pmt_id == 12 ? silverPrice * metal?.purity / 100 / 11.664 : 0)) * productData?.weight
    const rhodiumPrice = productData?.pmt_id == 11 ? 4 * silverPrice * productData?.weight / 11.664 : 0
    const makingCharges = productData?.mk_pp + (productData?.mk_gm * productData?.weight) + ((productData?.pmt_id <= 0 ? 0 : productData?.pmt_id < 11 ? goldPrice : silverPrice) / 11.664 * (productData?.jarti / 100) * productData?.weight) + ( productData?.jarti_gm * goldPrice / 11.664 )
    ...
    const actualPrice = productData?.isFixedPrice > 0 ? productData?.fixed_price :
        (metalPrice + makingCharges + ((productData?.pmt_id > 0 && productData?.pmt_id < 11 ? gold_tax : 0) / 100 * (metalPrice + makingCharges)) + rhodiumPrice + totalDiamondPrice)
