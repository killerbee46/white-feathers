...
export const updateMaterialPrice = (rates) => {
    const materialRates = rates?.rates
      Object?.keys(materialRates)?.map(async(key) => {
        ...
        switch (key) {
          case "gold":
            updateRateQuery = `update package_material set price = ${materialRates[key]}, last_updated_at = '${dayjs()}' where pm_id = 4`
          ...
          //   sequelize.query(updateRateQuery)
          //   updateRateQuery = `update package set fixed_price = ${materialRates[key]+250} where id_pack = 2292`
          //   sequelize.query(updateRateQuery)
          //   break
          default:
            break;
